...
use flow_core::models::{model_dir, model_files, stt_model, Precision, NEMO128_FILE, VOCAB_FILE};
use log::info;
...
pub fn import_from_hf_cache(id: &str) -> anyhow::Result<()> {
    ...
    if nemo.is_file() && verify_nemo128(&nemo, NEMO128_SHA256).is_ok() {
        ...
        let src = venv_nemo128().context(
            "nemo128.onnx not found: expected a virtualenv with onnx-asr at $FLOW_VENV, $VIRTUAL_ENV or ./.venv",
        )?;
