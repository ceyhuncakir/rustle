...
/// The key for a provider: environment first, then the keyring, then - on
/// Linux, once - the entry the Python version left behind.
pub fn get_key(provider: &str) -> String {
    ...
    match entry(provider).and_then(|e| e.get_password()) {
        ...
        Err(keyring::Error::NoEntry) => legacy_key(provider),
...
pub fn clear_key(provider: &str) -> bool {
    // The previous version's copy goes too, or the next start would find it
...
mod legacy {
    ...
    pub(super) fn migrate(
        ...
        if store(provider, &key) {
            info!("moved the {provider} API key over from the previous version of Flow");
        }
        Some(key)
    }
...
mod tests {
    ...
    fn a_missing_or_failing_secret_tool_is_not_an_error() {
        ...
        assert_eq!(legacy::run(&mut Command::new("flow-no-such-program-anywhere"), limit), None);
