...
use rustle_core::models::{is_downloaded, model_dir, Precision, DEFAULT_STT_MODEL};
use rustle_stt::{Parakeet, Transcription};
...
fn setup(precision: Precision) -> Option<Golden> {
    ...
    println!("running on {}", rustle_stt::runtime_info());
...
fn cpu_matches_python_golden() {
    ...
    let strict = rustle_stt::runtime_info() == golden.build_info;
    println!(
        "{}",
        if strict {
...
fn gpu_matches_python_golden_within_tolerance() {
    ...
    let backend = rustle_stt::gpu::backend().expect("a GPU build");
    let model = Parakeet::new(&golden.model, "gpu");
    model.load().expect("load on the GPU (rustle gpu says what is missing)");
    let report = model.compute_report().unwrap();
    println!("compute: requested={} actual={} ({})", report.requested, report.actual, report.reason);
