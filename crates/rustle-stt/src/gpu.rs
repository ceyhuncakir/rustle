...
use log::info;
use rustle_core::models::Precision;
use serde::Serialize;
...
impl GpuReport {
    /// One line for logs, `rustle doctor` and the compute report.
...
fn probe() -> GpuReport {
    ...
    let (gpu, blocker) = match backend() {
        ...
        None => (
            ...
            Some(blocked(
                "this build of Rustle has no GPU support",
...
fn cuda_blocker(nvidia: &Result<nvml::Nvidia, Option<Blocker>>, devices: &[GpuDevice]) -> Option<Blocker> {
    ...
    if nv.cuda < 12_000 {
        return Some(blocked(
            format!(
                "the NVIDIA driver ({}) supports CUDA {} but Rustle needs CUDA 12",
                nv.driver,
...
fn provider_blocker() -> Option<Blocker> {
    ...
    let fix = match real {
        Some(real) if real != dir && PROVIDER_LIBS.iter().all(|f| real.join(f).is_file()) => format!(
            "start Rustle as {} rather than through a link; scripts/install-app.sh sets this up",
            real.join(program_name()).display()
        ),
        _ => "reinstall Rustle with GPU support (scripts/install-app.sh)".into(),
    };
    Some(blocked(
...
mod vulkan {
    ...
    unsafe fn enumerate(lib: &Library) -> Result<Vec<Adapter>, String> {
        ...
        let app = ApplicationInfo {
            ...
            next: std::ptr::null(),
            application_name: c"Rustle".as_ptr(),
...
mod cuda_libs {
    ...
    pub fn blocker(missing: &[&Needed]) -> Option<Blocker> {
        ...
        let fix = if cfg!(windows) {
            "install the CUDA 12 toolkit and cuDNN 9 from developer.nvidia.com, then restart Rustle"
                .to_string()
        } else if names == ["cuDNN 9"] {
            "install cuDNN 9 for CUDA 12 (NVIDIA's libcudnn9-cuda-12 package, or `pip install --user nvidia-cudnn-cu12`), then restart Rustle".to_string()
        } else {
            format!(
                "install NVIDIA's CUDA 12 and cuDNN 9 packages, or `pip install --user {}`, then restart Rustle",
                wheels.join(" ")
            )
        };
    ...
    /// search path. `RUSTLE_CUDA_LIBS` (a path list) comes first.
    fn search_dirs() -> Vec<PathBuf> {
        let mut dirs: Vec<PathBuf> = Vec::new();
        if let Some(extra) = std::env::var_os("RUSTLE_CUDA_LIBS") {
...
mod tests {
    ...
    fn provider_dir_follows_argv0() {
        ...
        assert_eq!(dir_for_argv0(Path::new("/opt/rustle/rustle"), cwd, None), Some("/opt/rustle".into()));
        assert_eq!(
            dir_for_argv0(Path::new("target/debug/rustle"), cwd, None),
            Some("/home/me/target/debug".into())
        );
        assert_eq!(
            dir_for_argv0(Path::new("rustle-no-such-program"), cwd, Some(OsStr::new("/nowhere"))),
            None
        );
    }
}
