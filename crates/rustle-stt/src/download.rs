...
//! the account, changes what Rustle runs. `nemo128.onnx` is the exception: it
...
//! and the `.part` is locked while it is written, so a second Rustle process
...
use rustle_core::models::{model_dir, model_files, stt_model, Precision, NEMO128_FILE};
...
impl Net {
    fn new(stall: Duration) -> anyhow::Result<Net> {
        ...
        let client = Client::builder()
            .user_agent(concat!("rustle-stt/", env!("CARGO_PKG_VERSION")))
...
/// Take the `.part` for this process, so two Rustle processes never append to
...
fn lock_part(file: &File, part: &Path) -> anyhow::Result<()> {
    match file.try_lock() {
        ...
        Err(fs::TryLockError::WouldBlock) => {
            bail!("{} is being downloaded by another Rustle process", part.display())
        }
...
impl Stream {
    fn start(net: &Net, url: &str, from: u64) -> anyhow::Result<Stream> {
        ...
        std::thread::Builder::new().name("rustle-download-net".into()).spawn(move || {
...
mod tests {
    ...
    use rustle_core::models::STT_MODELS;
    ...
    fn a_part_locked_elsewhere_is_left_alone() {
        ...
        let err =
            fetch_to(&net, &url, &dest, &expect(&decoder), &AtomicBool::new(false), |_, _| {}).unwrap_err();
        assert!(err.to_string().contains("another Rustle process"), "{err}");
        assert!(!dest.exists());
        drop(other);
