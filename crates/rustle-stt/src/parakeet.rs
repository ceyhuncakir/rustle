...
use rustle_core::dsp;
use rustle_core::engine::Transcriber;
use rustle_core::models::{is_downloaded, model_dir, model_files, Precision, NEMO128_FILE, VOCAB_FILE};
...
pub struct Parakeet {
    ...
    /// dictation still finishing when Rustle was switched off or told to quit
        ...
        match provider {
            ...
            p if gpu::insists_on_gpu(p) => {
                let Some(device) = gpu_device() else {
                    bail!(
                        "provider = \"{provider}\" but this build of Rustle has no GPU support; \
...
fn unloaded() -> anyhow::Error {
    anyhow!("the speech model was unloaded (Rustle was switched off or is quitting)")
}
...
fn build_session(path: &Path, device: Device) -> anyhow::Result<Session> {
    if RUNTIME_RELEASED.load(Ordering::SeqCst) {
        bail!("ONNX Runtime has been shut down; Rustle is exiting");
    }
    keep_environment();
    ...
    let mut builder = match device {
        ...
        Device::Cuda => bail!("rustle-stt was built without the `cuda` feature"),
        #[cfg(feature = "webgpu")]
        ...
        Device::WebGpu => bail!("rustle-stt was built without the `webgpu` feature"),
        #[cfg(test)]
...
mod tests {
    ...
    fn fixture(name: &str) -> Option<(Vec<f32>, u32)> {
        let id = rustle_core::models::DEFAULT_STT_MODEL;
    ...
    fn on_cpu() -> &'static Parakeet {
        ...
        CPU.get_or_init(|| {
            let p = Parakeet::new(rustle_core::models::DEFAULT_STT_MODEL, "cpu");
    ...
    fn on_gpu() -> Parakeet {
        let p = Parakeet::new(rustle_core::models::DEFAULT_STT_MODEL, "gpu");
        match gpu_under_test() {
            ...
            _ => p.load().expect("load on the GPU (rustle gpu says what is missing)"),
        }
        p
    }
