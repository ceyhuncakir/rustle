//! GNOME Shell: everything goes through the Rustle Shell extension.
//!
...
//! them on the session bus as `dev.ceyhun.Rustle.Island`. This module is the client.
...
#[zbus::proxy(
    interface = "dev.ceyhun.Rustle.Island",
    default_service = "dev.ceyhun.Rustle.Island",
    default_path = "/dev/ceyhun/Rustle/Island",
    gen_async = false
)]
trait Island {
...
///
/// At login `rustle --headless` can start before the Shell has loaded its
...
/// The version of the extension this build of Rustle carries, from its
...
/// copy this build carries. The Shell keeps running an old copy after Rustle
...
impl GnomeIsland {
    ...
    pub fn connect() -> anyhow::Result<GnomeIsland> {
        if !extension_present() {
            anyhow::bail!(
                "{BUS_NAME} is not on the session bus - is the Rustle extension enabled? \
                 (gnome-extensions enable rustle@ceyhun.dev, then log out and back in)"
            );
        }
