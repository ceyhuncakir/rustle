...
//! reach Rustle: their own key bindings run `rustle hotkey down` on press and
//! `rustle hotkey up` on release. It runs on every Linux session, next to the
//! real shortcut, so scripts work everywhere.
//!
//! The socket sits in `$XDG_RUNTIME_DIR/rustle/`, a directory only this user
...
use rustle_core::engine::{DesktopError, Event, Hotkey, HotkeyEvent};
...
/// Whether any key binding has reached Rustle through the socket in this
...
/// `$XDG_RUNTIME_DIR/rustle/control.sock`, or `None` without a runtime dir.
...
pub fn socket_path_in(runtime_dir: &Path) -> PathBuf {
    runtime_dir.join("rustle").join("control.sock")
}
...
/// Whether a running Rustle answers on the socket.
...
impl Drop for ControlSocket {
    /// `rustle --headless` lets its engine go without stopping it; the socket
...
/// Clear the way for our socket. One that still answers belongs to a Rustle
/// that is running; one that does not was left by a Rustle that died.
...
fn clear_stale(path: &Path) -> io::Result<()> {
    ...
    if UnixStream::connect(path).is_ok() {
        return Err(io::Error::new(ErrorKind::AddrInUse, "another Rustle is already listening"));
    }
    fs::remove_file(path)
}
...
pub enum SendError {
    #[error(
        "Rustle is not running: nothing listens on {0}. Start it from the app menu, or with `rustle --headless`."
    )]
    NotRunning(PathBuf),
    #[error("$XDG_RUNTIME_DIR is not set, so there is no control socket to reach Rustle on")]
    NoRuntimeDir,
    #[error("Rustle said: {0}")]
    Refused(String),
...
/// Send one command to the running Rustle and wait for its answer.
...
mod tests {
    ...
    fn a_stale_socket_is_replaced() {
        ...
        // Bound and dropped: the file stays, nobody listens. A Rustle that died.
    ...
    fn a_linked_directory_is_refused() {
        ...
        std::os::unix::fs::symlink(elsewhere.path(), runtime.path().join("rustle")).unwrap();
        let (tx, _rx) = mpsc::channel();
    ...
    fn nobody_listening_says_flow_is_not_running() {
        ...
        assert!(err.to_string().contains("Rustle is not running"));
    }
