...
use rustle_core::engine::{Focus, Hotkey, Injector, Overlay};
...
pub enum HotkeySource {
    ...
    /// can run `rustle hotkey ...`, which reaches the engine through the
...
pub struct Backends {
    ...
    /// Human-readable notes for `rustle doctor`: what was chosen and why.
