...
pub enum Session {
    ...
    MacOs,
    X11,
    /// GNOME Shell, served by the Rustle Shell extension when `extension` says
...
impl fmt::Display for Session {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Session::GnomeWayland { extension: true } => write!(f, "GNOME Shell (Rustle extension present)"),
            Session::GnomeWayland { extension: false } => {
                write!(f, "GNOME Wayland (Rustle extension missing)")
            }
...
pub struct LinuxEnv {
    pub rustle_desktop: Option<String>,
    pub session_type: Option<String>,
    pub current_desktop: Option<String>,
    pub wayland_display: Option<String>,
    /// Whether `dev.ceyhun.Rustle.Island` has an owner on the session bus.
...
impl LinuxEnv {
    pub fn from_process() -> LinuxEnv {
        ...
        let mut this = LinuxEnv {
            rustle_desktop: env("RUSTLE_DESKTOP"),
        ...
        #[cfg(target_os = "linux")]
        {
            let looks_like_gnome = this
                ...
                .unwrap_or(false);
            if looks_like_gnome || this.rustle_desktop.as_deref() == Some("gnome") {
...
pub fn detect_linux(env: &LinuxEnv) -> Session {
    if let Some(forced) = env.rustle_desktop.as_deref() {
        match forced {
            ...
            other => log::warn!("RUSTLE_DESKTOP={other:?} not understood, detecting instead"),
        }
    }
...
mod tests {
    ...
    fn override_wins() {
        let mut e = env("wayland", "GNOME", true);
        e.rustle_desktop = Some("x11".into());
