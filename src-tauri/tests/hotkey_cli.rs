//! `rustle hotkey` end to end: the real binary reaches a control socket in a
...
use rustle_core::engine::{Event, Hotkey, HotkeyEvent};
use rustle_desktop::linux::control::{socket_path_in, ControlSocket};

fn rustle(runtime: &Path, args: &[&str]) -> Output {
...
fn every_command_reaches_a_running_flow() {
    ...
    for action in ["down", "up", "toggle", "cancel"] {
        let out = rustle(runtime.path(), &["hotkey", action]);
...
fn without_a_running_flow_it_says_so() {
    let runtime = tempfile::tempdir().unwrap();
    let out = rustle(runtime.path(), &["hotkey", "toggle"]);
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("Rustle is not running"), "{stderr}");
}

#[test]
fn an_unknown_action_is_refused_before_anything_is_sent() {
    let runtime = tempfile::tempdir().unwrap();
    let out = rustle(runtime.path(), &["hotkey", "explode"]);
